...
WEZTERM_THEME_DIR="$DOTFILES_DIR/.config/wezterm/themes/$THEME"

if [[ ! -d "$WEZTERM_THEME_DIR" ]]; then
    error "WezTerm theme not found: $THEME"
    error "Available themes:"
    ls -1 "$DOTFILES_DIR/.config/wezterm/themes/" 2>/dev/null | sed 's/^/  - /'
    exit 1
fi
...
if [[ -f "$WEZTERM_THEME_DIR/wezterm.lua" ]]; then
    cp "$WEZTERM_THEME_DIR/wezterm.lua" "$DOTFILES_DIR/.config/wezterm/wezterm.lua"
...
if [[ -d "$WEZTERM_THEME_DIR/assets" ]]; then
    ...
    cp -r "$WEZTERM_THEME_DIR/assets/"* "$DOTFILES_DIR/.config/wezterm/assets/" 2>/dev/null || true
fi
